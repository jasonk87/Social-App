...
def list_model_profiles() -> List[dict]:
    ...
    try:
        ...
        return [profile for profile in profiles if profile['completion']]
...
def get_ai_settings() -> dict:
    with get_db_connection() as conn:
        settings = dict(conn.execute('SELECT model,thinking_enabled,thinking_mode FROM ai_settings WHERE id=1').fetchone())
