...
ACTIVITY_INTERVALS = (0, 30, 60, 300, 900, 1800)
AI_ACTIVITY_EVENTS = ('COMMUNITY_POST', 'AGENT_REPLY', 'AGENT_EVOLVE',
                      'AGENT_FOUND_COMMUNITY', 'COMMUNITY_SCHISM', 'COMMUNITY_MERGER',
                      'AGENT_MODERATE', 'AMA_START', 'AMA_END')
...
def init_db() -> None:
    ...
    try:
        ...
        # One model for the whole installation. Keep legacy columns as mirrors
        # for older clients; they are never authoritative for generation.
        cur.execute('''CREATE TABLE IF NOT EXISTS ai_settings (
            id INTEGER PRIMARY KEY CHECK (id = 1), model TEXT NOT NULL,
            thinking_enabled INTEGER NOT NULL DEFAULT 1 CHECK (thinking_enabled IN (0,1)),
            thinking_mode TEXT NOT NULL DEFAULT 'unknown')''')
        ai_columns = {row[1] for row in cur.execute('PRAGMA table_info(ai_settings)')}
        if 'activity_interval_seconds' not in ai_columns:
            cur.execute('ALTER TABLE ai_settings ADD COLUMN activity_interval_seconds INTEGER NOT NULL DEFAULT 60')
        if 'next_activity_at' not in ai_columns:
            cur.execute('ALTER TABLE ai_settings ADD COLUMN next_activity_at REAL NOT NULL DEFAULT 0')
        existing = cur.execute("SELECT model FROM communities WHERE model NOT IN ('','human','none') GROUP BY model ORDER BY COUNT(*) DESC, MIN(id) LIMIT 1").fetchone()
        cur.execute('INSERT OR IGNORE INTO ai_settings (id,model) VALUES (1,?)',
                    (existing[0] if existing else 'llama3.1:8b',))
        cur.execute('UPDATE communities SET model=(SELECT model FROM ai_settings WHERE id=1)')
        cur.execute('UPDATE communities SET posting_rate=MAX(30,(SELECT activity_interval_seconds FROM ai_settings WHERE id=1))')
        cur.execute("UPDATE agents SET model=(SELECT model FROM ai_settings WHERE id=1) WHERE model NOT IN ('none','human')")
...
def model_profile(item: Dict[str, Any]) -> dict:
    ...
    architecture = data.get('model_info', {}).get('general.architecture', '')
    families = data.get('details', {}).get('families') or []
    level_only = architecture == 'gptoss' or 'gptoss' in families or item['name'].split(':')[0] == 'gpt-oss'
    profile = {
        'name': item['name'],
        'display_name': {'gemma4:e2b': 'Gemma 4 E2B', 'gemma4:e4b': 'Gemma 4 E4B'}.get(item['name'], item['name']),
        'completion': capabilities is None or 'completion' in capabilities,
        'thinking_mode': ('levels' if level_only else 'unknown' if capabilities is None
                          else 'toggle' if 'thinking' in capabilities else 'none'),
    }
    with MODEL_CAPABILITIES_LOCK:
        MODEL_CAPABILITIES[key] = profile
    return profile
...
def list_model_profiles() -> List[dict]:
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=min(8, len(items))) as pool:
            profiles = list(pool.map(model_profile, items))
        # Explain the locally installed 4b alias only when Ollama confirms it
        # points to the same content as E4B. Preserve every exact tag as a value.
        digests = {item['name']: item.get('digest') for item in items}
        result = [dict(profile) for profile in profiles if profile['completion']]
        if digests.get('gemma4:e4b') and digests.get('gemma4:4b') == digests['gemma4:e4b']:
            for profile in result:
                if profile['name'] == 'gemma4:4b':
                    profile['display_name'] = 'Gemma 4 E4B (gemma4:4b alias)'
        return result
...
def list_models() -> List[str]:
    return [profile['name'] for profile in list_model_profiles()]


def get_ai_settings() -> dict:
    with get_db_connection() as conn:
        settings = dict(conn.execute('SELECT model,thinking_enabled,thinking_mode,activity_interval_seconds FROM ai_settings WHERE id=1').fetchone())
    settings['thinking_enabled'] = bool(settings['thinking_enabled'])
    return settings


def save_activity_settings(interval: int) -> dict:
    if type(interval) is not int or interval not in ACTIVITY_INTERVALS:
        raise ValueError('Choose a supported activity interval or pause the bots.')
    with get_db_connection() as conn:
        conn.execute('BEGIN IMMEDIATE')
        previous = conn.execute('SELECT activity_interval_seconds FROM ai_settings WHERE id=1').fetchone()[0]
        if previous != interval:
            now = time.time()
            conn.execute('UPDATE ai_settings SET activity_interval_seconds=?,next_activity_at=? WHERE id=1',
                         (interval, now + interval if interval else 0))
            conn.execute('UPDATE communities SET posting_rate=?', (max(30, interval),))
            # A faster setting should also bring long-waiting work forward.
            if interval:
                placeholders = ','.join('?' for _ in AI_ACTIVITY_EVENTS)
                conn.execute(f"UPDATE simulation_events SET timestamp=MIN(timestamp,?) WHERE status='pending' AND event_type IN ({placeholders})",
                             (now + interval, *AI_ACTIVITY_EVENTS))
    settings = get_ai_settings()
    broadcast_sse('ai_settings_changed', settings)
    return settings


def save_ai_settings(model: str, thinking_enabled: bool) -> dict:
    model = coerce_string(model, 'model', required=True, max_length=200)
    thinking_enabled = parse_bool(thinking_enabled, 'thinking_enabled')
    profile = next((p for p in list_model_profiles() if p['name'] == model), None)
    if profile is None:
        raise ValueError('Choose an installed Ollama text model. Refresh the model list and try again.')
    mode = profile['thinking_mode']
    if not thinking_enabled and mode in ('levels', 'unknown'):
        raise ValueError('This model cannot turn thinking off.' if mode == 'levels'
                         else 'Ollama does not report thinking controls for this model.')
    with get_db_connection() as conn:
        conn.execute('BEGIN IMMEDIATE')
        conn.execute('UPDATE ai_settings SET model=?,thinking_enabled=?,thinking_mode=? WHERE id=1',
                     (model, int(thinking_enabled), mode))
        conn.execute('UPDATE communities SET model=?', (model,))
        conn.execute("UPDATE agents SET model=? WHERE model NOT IN ('none','human')", (model,))
    # Jobs already queued still resolve the model at the common generation boundary.
...
def generate_text(model: str, prompt: str, system: Optional[str] = None, max_tokens: Optional[int] = None, json_output: Any = False, temperature: float = 0.85) -> str:
    """Generate text using the installation's current AI settings.

    Args:
        model: Legacy argument, ignored so old queued jobs cannot override settings.
    ...
    """
    settings = get_ai_settings()
    thinking = settings['thinking_enabled'] and settings['thinking_mode'] in ('toggle', 'levels')
    payload: Dict[str, Any] = {
        'model': settings['model'],
        'prompt': prompt,
        'stream': False,
        'options': {'num_predict': (max_tokens or 512) + (2048 if thinking else 0), 'temperature': temperature,
                    'top_p': 0.9, 'repeat_penalty': 1.12, 'num_ctx': 8192},
    }
    if settings['thinking_mode'] == 'toggle':
        payload['think'] = settings['thinking_enabled']
    elif settings['thinking_mode'] == 'levels':
...
def review_factual_claims(model, data, context, description):
    if not quality.needs_fact_review(data, context['plan']['mode'], description):
        return ''
    reviewer = get_ai_settings()['model']
...
class SimulationEngine:
    ...
    def _claim_next_due_event(self):
        ...
        try:
            ...
            cur = conn.cursor()
            now = time.time()
            pace = cur.execute('SELECT activity_interval_seconds,next_activity_at FROM ai_settings WHERE id=1').fetchone()
            activity_ready = pace['activity_interval_seconds'] > 0 and now >= pace['next_activity_at']
            placeholders = ','.join('?' for _ in AI_ACTIVITY_EVENTS)
            cur.execute(
                f"SELECT id, event_type, event_data FROM simulation_events WHERE status='pending' AND timestamp<=? AND (? OR event_type NOT IN ({placeholders})) ORDER BY timestamp ASC, priority ASC, id ASC LIMIT 1",
                (now, int(activity_ready), *AI_ACTIVITY_EVENTS)
            )
            row = cur.fetchone()
            if row:
                cur.execute(
                ...
                if cur.rowcount > 0:
                    if row['event_type'] in AI_ACTIVITY_EVENTS:
                        cur.execute('UPDATE ai_settings SET next_activity_at=? WHERE id=1',
                                    (now + pace['activity_interval_seconds'],))
                    conn.commit()
    ...
    def _finish_activity_turn(self):
        # Leave a full quiet interval after slow generation or an error. The
        # persisted deadline also prevents a burst of overdue jobs on restart.
        with get_db_connection() as conn:
            conn.execute('UPDATE ai_settings SET next_activity_at=MAX(next_activity_at,?+activity_interval_seconds) WHERE id=1 AND activity_interval_seconds>0',
                         (time.time(),))
        ...
        while not self._stop_event.is_set():
            ...
            if event:
                try:
                    ...
                    if event.event_type in AI_ACTIVITY_EVENTS:
                        self._finish_activity_turn()
            else:
                time.sleep(1)
...
class Simulation:
    ...
    def effective_posting_rate(self) -> int:
        return max(30, get_ai_settings()['activity_interval_seconds'])
...
def add_community(name: str, description: str, model: str, posting_rate: int, tone: str = DEFAULT_COMMUNITY_TONE, style_notes: str = "") -> int:
    ...
    try:
        conn.execute('BEGIN IMMEDIATE')
        model = conn.execute('SELECT model FROM ai_settings WHERE id=1').fetchone()[0]
        posting_rate = max(30, conn.execute('SELECT activity_interval_seconds FROM ai_settings WHERE id=1').fetchone()[0])
        cur = conn.cursor()
        cur.execute(
    ...
    def handle_api_get(self, parsed) -> None:
        ...
        try:
            if path == 'stream':
                ...
                self.respond_json({'models': [p['name'] for p in profiles], 'model_details': profiles})
            elif path == 'ai-settings':
                self.respond_json({'settings': get_ai_settings()})
    ...
    def handle_api_post(self, parsed) -> None:
        ...
        try:
            if path == 'login':
                ...
                if self.require_current_user() is None:
                    return
                settings = save_ai_settings(data.get('model'), data.get('thinking_enabled'))
                self.respond_json({'success': True, 'settings': settings})
            elif path == 'activity-settings':
                if self.require_current_user() is None:
                    return
                self.respond_json({'success': True, 'settings': save_activity_settings(data.get('activity_interval_seconds'))})
            elif path == 'communities':
